  ...
  if (!district) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-4xl font-serif font-bold text-gray-900 mb-4">404: District Not Found</h1>
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-6">
            <div>
              ...
              <h2 className="text-2xl md:text-3xl font-light text-gray-500 mt-2 italic">
                {district.tagline}
              </h2>
            </div>
            ...
            <div className="flex flex-wrap gap-2 md:max-w-[300px] justify-start md:justify-end">
              {district.pillars.map(p => (
                <span key={p} className="px-3 py-1 bg-gray-200 text-gray-700 text-xs font-bold rounded-full border border-gray-300">
                  {p}
                </span>
              ))}
            </div>
          ...
          <p className="text-xl text-gray-700 leading-relaxed max-w-4xl border-l-4 border-crimson pl-6 mt-10">
            {district.summary}
          </p>
        </motion.div>
      </section>
          ...
          <div className="space-y-6">
            {district.keyCrises.map((crisis, i) => (
              <DataCard key={i} title={`${crisis.year} — ${crisis.title}`} alert={crisis.status === 'danger' || true}>
                <p className="text-gray-600 leading-relaxed">{crisis.detail}</p>
              </DataCard>
            ))}
          </div>
        </Section>
