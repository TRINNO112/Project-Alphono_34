      ...
      <ul className="space-y-2.5">
        {rows.map((r) => {
          ...
          return (
            ...
            >
              <span className="text-xs font-semibold text-gray-600 truncate" title={r.label}>
                {r.label}
              </span>
              ...
              <span className="text-xs font-mono tabular-nums text-right text-gray-700">
                {r.value}%
              </span>
              {hasWhy ? (
                <button
              ...
              <AnimatePresence>
                {hasWhy && openWhy === r.id && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full z-30 mt-1 w-80 max-h-[60vh] overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-xl p-4"
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h4 className="font-serif text-base font-bold text-gray-900 leading-tight break-words">
                        Why {r.value}%?
                      </h4>
                      <button
                        type="button"
                    ...
                    {top.derivation?.formula && (
                      <div className="mb-3 p-2 rounded-lg bg-gray-50 border border-gray-100">
                        <code className="text-xs font-mono text-gray-700 break-words whitespace-pre-wrap">
                          {top.derivation.formula}
                        </code>
                      </div>
                    )}
                      ...
                      <dl className="space-y-1.5 mb-3">
                        {top.derivation.factors.map((f, i) => (
                          <div key={i} className="flex items-baseline justify-between gap-3 text-xs">
                            <dt className="text-gray-600 flex-1 min-w-0 break-words">
                              {f.label}
                            </dt>
                    ...
                    <p className="text-[11px] text-gray-500 leading-relaxed pt-2 border-t border-gray-100">
                      Driver: <span className="font-semibold text-gray-700 break-words">{top.label}</span>
                      {' '}at lever{' '}
                      <span className="font-mono tabular-nums">{top.leverValue}/{top.leverMax}</span>.
                      {extraCount > 0 && (
                        <span className="block mt-1 text-gray-400">+{extraCount} more lever{extraCount > 1 ? 's' : ''} contributing</span>
                      )}
                    </p>
                  </motion.div>
                )}
